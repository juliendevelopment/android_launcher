...
class GridRepository @Inject constructor(
    ...
    val layout: Flow<GridLayout> = combine(
        dao.observeItems(),
        dao.observeFolders(),
        ...
        val mapped: List<GridItem> = items.mapNotNull { entity ->
            val coords = runCatching { GridCoords(entity.cellX, entity.cellY) }.getOrNull()
            ...
            when (entity.type) {
                ...
                GridItemType.FOLDER -> {
                    ...
                    val contained = itemsByFolderId[folderId].orEmpty()
                ...
                GridItemType.WIDGET -> {
                    ...
                    val span = runCatching { GridSpan(entity.spanX, entity.spanY) }.getOrNull()
                        ?: return@mapNotNull null
    ...
    suspend fun placeApp(app: AppEntry, target: GridCoords): Long =
        dao.insertAppShortcut(app.componentFlattened, target.x, target.y)

    suspend fun placeWidget(appWidgetId: Int, providerFlattened: String, target: GridCoords, span: GridSpan): Long {
        dao.insertWidget(WidgetEntity(appWidgetId, providerFlattened))
        return dao.insertItem(
            GridItemEntity(
                cellX = target.x, cellY = target.y, spanX = span.width, spanY = span.height,
                type = GridItemType.WIDGET,
                componentFlattened = null, folderId = null, appWidgetId = appWidgetId,
            ),
        )
    }

    suspend fun createFolder(target: GridCoords, contents: List<AppEntry>): Long {
        ...
        return dao.insertItem(
            GridItemEntity(
                cellX = target.x, cellY = target.y, spanX = 1, spanY = 1,
                type = GridItemType.FOLDER,
                componentFlattened = null, folderId = folderId, appWidgetId = null,
            ),
        )
    }

    suspend fun addAppToFolder(folderItemId: Long, app: AppEntry) {
    ...
    suspend fun removeAppFromFolder(folderItemId: Long, app: AppEntry) {
        val item = dao.itemById(folderItemId) ?: return
        val folderId = item.folderId ?: return
    ...
    suspend fun moveItem(itemId: Long, target: GridCoords) {
